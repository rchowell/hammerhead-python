...
from hammerhead import Hammerhead, AsyncHammerhead, DefaultAioHttpClient
from hammerhead._utils import is_dict
...
logging.getLogger("hammerhead").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Hammerhead]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Hammerhead(
        base_url=base_url, webhook_signature=webhook_signature, _strict_response_validation=strict
    ) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncHammerhead]:
